...
class _SearchTabState extends ConsumerState<SearchTab> {
  ...
  void initState() {
    ...
    // 📜 Listener para detectar cuando se hace scroll en las sugerencias
    _suggestionsScrollController.addListener(() {
      final shouldShow = _suggestionsScrollController.offset > 0;
      if (_showGradient != shouldShow) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final latestReleases = ref.watch(latestReleasesProvider);
    ...
    return Column(
      children: [
        ...
        // 📄 Área donde muestro los resultados de la búsqueda o las sugerencias
        Expanded(
          child: GestureDetector(
            ...
            child: hasActiveSearch
                ...
                : _buildSuggestions(latestReleases),
          ),
        ),
      ],
    );
  ...
  Widget _buildSearchResults(AsyncValue<List<Game>> searchResults) {
  ...
  /// 🆕 Construye la vista de sugerencias (últimos lanzamientos)
  Widget _buildSuggestions(AsyncValue<List<Game>> latestReleases) {
    return latestReleases.when(
      data: (games) {
        if (games.isEmpty) {
          return Center(
            child: Text(
        ...
        return Column(
          children: [
            ...
            // 📐 GridView con los juegos y gradiente sutil
            Expanded(
              child: Stack(
                children: [
                  GridView.builder(
                  ...
                  // 🎨 Gradiente sutil en la parte superior (solo cuando se hace scroll)
                  if (_showGradient)
                    Positioned(
                      top: 0,
                      left: 0,
